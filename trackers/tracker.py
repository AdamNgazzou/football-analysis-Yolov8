...
class Tracker : 
    ...
    def detect_frames(self, frames) :
        ...
        for i in range(0, len(frames), BATCH_SIZE) :
            ...
            detection_batch = self.model.predict(source=batch_frames, conf=0.1)
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None) :
        ...
        tracks = {
            "players" : [], # [{track_id : {"bbox" : [x1,y1,x2,y2]}}, {track_id1 : {"bbox" : [x1,y1,x2,y2]}} , ...] 
            "referees" : [],
            "ball" : []
        } 
        ...
        for frame_num, detection in enumerate(detections) : 
            ...
            for frame_detection in detections_with_tracks :
                ...
                if class_id == cls_name_inv["player"] :
                    tracks["players"][frame_num][track_id] = {"bbox" : bbox}
                if class_id == cls_name_inv["referee"] :
                    tracks["referees"][frame_num][track_id] = {"bbox" : bbox}
            
            for frame_detection in detection_supervision : 
                bbox = frame_detection[0].tolist()
                class_id = frame_detection[3]
                if class_id == cls_name_inv["ball"] :
    ...
    def draw_team_ball_control(self, frame, frame_num, team_ball_control) :
        ...
        # Get the number of times each team has had ball control
